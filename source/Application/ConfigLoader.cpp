...
std::expected<LaunchSettings, std::string> LoadLaunchSettingsFromAppDirectory()
{
    ...
    readOpt<int64_t, int> (config, "RefWidth",              settings.RefWidth);
    readOpt<int64_t, int> (config, "RefHeight",             settings.RefHeight);
    readOpt<double, float>(config, "PhysicsPixelsPerMeter", settings.PhysicsPixelsPerMeter);
    readOpt<double, float>(config, "PhysicsDefaultGravityX",settings.PhysicsDefaultGravityX);
    readOpt<double, float>(config, "PhysicsDefaultGravityY",settings.PhysicsDefaultGravityY);
